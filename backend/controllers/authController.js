...
const googleCallback = async (accessToken, refreshToken, profile, done) => {
  try {
    ...
    if (rows.length === 0) return done(null, false, { message: "Email not registered." });

    const user = rows[0];

    // Subtract 5h 30m manually
    user.deadline = moment(user.deadline).subtract(5, 'hours').subtract(30, 'minutes').toDate();
    if (user.first_login) {
      user.first_login = moment(user.first_login).subtract(5, 'hours').subtract(30, 'minutes').toDate();
    }
...
const checkUserTimeValidity = async (req, res) => {
  try {
    ...
    const deadline = moment(user.deadline).subtract(5, 'hours').subtract(30, 'minutes');
    const firstLogin = moment(user.first_login).subtract(5, 'hours').subtract(30, 'minutes');
